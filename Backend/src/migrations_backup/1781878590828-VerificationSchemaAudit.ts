...
export class VerificationSchemaAudit1781878590828 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_shop_visits_distributor" ON "shop_visits" USING btree ("distributor_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_shop_visits_working_day" ON "shop_visits" USING btree ("working_day_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_shop_visits_salesman" ON "shop_visits" USING btree ("salesman_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_shop_visits_shop" ON "shop_visits" USING btree ("shop_id") `,
    );
    await queryRunner.query(
      `ALTER TABLE "shop_visits" ADD CONSTRAINT "FK_159a428a7a1279c22e746db2538" FOREIGN KEY ("working_day_id") REFERENCES "working_days"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
