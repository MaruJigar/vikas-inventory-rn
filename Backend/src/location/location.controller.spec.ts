...
describe('LocationController', () => {
  ...
  it('getLiveLocation', async () => {
    ...
    expect(service.getLiveLocation).toHaveBeenCalledWith('u1', 'DISTRIBUTOR_ADMIN', 's1');
  });

  it('getLocationHistory', async () => {
    const req = { user: { userId: 'u1', role: 'DISTRIBUTOR_ADMIN' } };
    await controller.getLocationHistory(req, 's1');
    expect(service.getLocationHistory).toHaveBeenCalledWith('u1', 'DISTRIBUTOR_ADMIN', 's1');
  });
});
