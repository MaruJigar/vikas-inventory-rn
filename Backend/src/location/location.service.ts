...
export class LocationService {
  ...
  async uploadLocation(userId: string, dto: CreateLocationDto) {
    ...
    if (!salesman)
      throw new ForbiddenException('Only salesmen can upload locations');
    ...
    if (!activeWd)
      throw new ForbiddenException(
        'Location tracking requires an active check-in',
      );
    ...
    if (dto.idempotency_key) {
      const existing = await this.locLogRepo.findOne({
        where: { sync_status: dto.idempotency_key },
      });
    ...
    await this.updateLatestLocation(
      salesman.id,
    ...
    if (dist) {
      this.socketGateway.broadcastToRoom(
        `DISTRIBUTOR_ADMIN:${dist.user_id}`,
        'LOCATION_UPDATED',
        {
    ...
    try {
      ...
      for (const loc of sorted) {
        ...
        if (loc.idempotency_key) {
          const existing = await queryRunner.manager.findOne(LocationLog, {
            where: { sync_status: loc.idempotency_key },
          });
      ...
      if (latestPoint) {
        ...
        if (
          ...
          maxDate.getTime() > latest.last_updated_at.getTime()
        ) {
  ...
  private async updateLatestLocation(
    salesmanId: string,
    distId: string,
    wdId: string,
    point: any,
    ...
    let latest = await this.latestLocRepo.findOne({
    ...
    if (
      !latest.last_updated_at ||
      capturedAt.getTime() > latest.last_updated_at.getTime()
    ) {
  ...
  async getLiveLocation(userId: string, userRole: string, salesmanId: string) {
    ...
    if (userRole === 'SUPER_ADMIN') {
      ...
      const manufacturer = await this.mfrRepo.findOne({
        where: { user_id: userId },
      });
      if (!manufacturer)
      ...
      if (!isLinked) {
        throw new ForbiddenException(
          'Cannot access location of salesmen outside your distributor ecosystem',
        );
      }
    }
  ...
  async getLocationHistory(
    ...
    if (userRole === 'SUPER_ADMIN') {
      ...
      if (!dist || dist.id !== salesman.distributor_id) {
        throw new ForbiddenException(
          'Cannot access location history of this salesman',
        );
      }
    } else if (userRole === 'MANUFACTURER_ADMIN') {
      const manufacturer = await this.mfrRepo.findOne({
      ...
      if (!isLinked) {
        throw new ForbiddenException(
          'Cannot access location history of salesmen outside your distributor ecosystem',
        );
      }
    }
