...
describe('LocationService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(LatestLocation),
          useValue: mockLatestLocationRepo,
        },
  ...
  describe('uploadLocation', () => {
    ...
    it('Should process location correctly', async () => {
      ...
      mockWorkingDayRepo.findOne.mockResolvedValue({
        id: 'wd1',
        salesman_id: 's1',
        status: 'ACTIVE',
      });
    ...
    it('Should update existing latest location correctly', async () => {
      ...
      mockLatestLocationRepo.findOne.mockResolvedValue({
        salesman_id: 's1',
      ...
      const dto = {
        latitude: 10,
        longitude: 20,
        captured_at: newDate.toISOString(),
      };
    ...
    it('Should skip socket update if distributor missing', async () => {
      ...
      await service.uploadLocation('u1', {
        latitude: 10,
        longitude: 20,
        captured_at: new Date().toISOString(),
      });
  ...
  describe('batchUploadLocations', () => {
    it('Should reject if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(
        service.batchUploadLocations('u1', { locations: [] }),
      ).rejects.toThrow(ForbiddenException);
    });
        ...
        locations: [
          {
            latitude: 10,
            longitude: 20,
            captured_at: new Date('2023-01-01').toISOString(),
          },
          {
            latitude: 11,
            longitude: 21,
            captured_at: new Date('2023-01-02').toISOString(),
          },
        ],
      };
    ...
    it('Should rollback on error', async () => {
      ...
      await expect(service.batchUploadLocations('u1', dto)).rejects.toThrow(
        'DB Error',
      );
    ...
    it('Should skip latest location update if batch is older', async () => {
      ...
      mockQueryRunner.manager.findOne.mockResolvedValueOnce({
        salesman_id: 's1',
        last_updated_at: new Date('2023-01-05'),
      }); // latest
      mockQueryRunner.manager.save.mockResolvedValue([]);
  ...
  describe('getLiveLocation', () => {
    it('Should reject if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getLiveLocation('u1', 'DISTRIBUTOR_ADMIN', 's1'),
    ...
    it('Should return location for distributor', async () => {
      ...
      const res = await service.getLiveLocation(
        'u1',
        'DISTRIBUTOR_ADMIN',
        's1',
      );
    ...
    it('Should reject if manufacturer admin does not own distributor', async () => {
      ...
      await expect(
        service.getLiveLocation('u1', 'MANUFACTURER_ADMIN', 's1'),
      ).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('getLocationHistory', () => {
    it('Should reject if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getLocationHistory('u1', 'DISTRIBUTOR_ADMIN', 's1'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should return history', async () => {
      ...
      const res = await service.getLocationHistory(
        'u1',
        'DISTRIBUTOR_ADMIN',
        's1',
      );
    ...
    it('Should reject if manufacturer admin does not own distributor in history', async () => {
      ...
      mockManufacturerDistributorRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getLocationHistory('u1', 'MANUFACTURER_ADMIN', 's1'),
      ).rejects.toThrow(ForbiddenException);
    });
