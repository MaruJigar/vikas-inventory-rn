...
export class DatabaseStatsTool {
  ...
  @Tool({
    ...
    description: 'Retrieves the total count of registered users, products, and distributors inside the Vikas Inventory system.',
  })
  async execute() {
    try {
      ...
      const distributorCount = await this.dataSource.query('SELECT COUNT(*) FROM distributors');
      const shopCount = await this.dataSource.query('SELECT COUNT(*) FROM shops');
