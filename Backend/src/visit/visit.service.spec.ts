...
describe('VisitService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(WorkingDay),
          useValue: mockWorkingDayRepo,
        },
        ...
        },
        {
          ...
          useValue: mockManufacturerDistributorRepo,
        },
  ...
  describe('startVisit', () => {
    it('Should throw if salesman not found', async () => {
      ...
      await expect(service.startVisit('u1', { shopId: 's1' })).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('Should throw NotFoundException if shop not found', async () => {
      ...
      await expect(
        service.startVisit('u1', { shopId: 'shop1' }),
    ...
    it('Should return existing visit on duplicate idempotency key', async () => {
      ...
      const res = await service.startVisit('u1', {
        shopId: 'shop1',
    ...
    it('Should reject future startedAt timestamp', async () => {
      ...
      await expect(
        service.startVisit('u1', { shopId: 'shop1', startedAt: futureTs }),
    ...
    it('Should reject invalid (NaN) startedAt timestamp', async () => {
      ...
      await expect(
        service.startVisit('u1', { shopId: 'shop1', startedAt: 'not-a-date' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should start visit with GPS coordinates and emit socket', async () => {
      ...
      mockVisitRepo.findOne.mockResolvedValue(null);
      mockVisitRepo.create.mockReturnValue({
        id: 'v1',
        started_at: new Date(),
      });
      mockVisitRepo.save.mockResolvedValue({
        id: 'v1',
        started_at: new Date(),
      });
  ...
  describe('endVisit', () => {
    it('Should throw if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(service.endVisit('u1', { visitId: 'v1' })).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('Should reject future endedAt timestamp', async () => {
      ...
      await expect(
        service.endVisit('u1', { visitId: 'v1', endedAt: futureTs }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should reject invalid (NaN) endedAt timestamp', async () => {
      ...
      mockOrderRepo.count.mockResolvedValue(1);
      await expect(
        service.endVisit('u1', { visitId: 'v1', endedAt: 'not-a-date' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should reject endedAt before startedAt (impossible timeline)', async () => {
      ...
      await expect(
        service.endVisit('u1', { visitId: 'v1', endedAt: beforeStart }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('Should close visit successfully with GPS and emit socket', async () => {
      ...
      const res = await service.endVisit('u1', {
        visitId: 'v1',
        latitude: 12.9,
        longitude: 77.5,
      });
    ...
    it('Should close visit with explicit valid past endedAt', async () => {
      ...
      mockVisitRepo.findOne.mockResolvedValue({
        ...
        distributor_id: 'd1',
        no_order_reason: null,
        started_at: new Date('2020-01-01T08:00:00Z'),
      });
      mockOrderRepo.count.mockResolvedValue(1);
      mockVisitRepo.save.mockResolvedValue({ id: 'v1', status: 'CLOSED' });
  ...
  describe('noOrderVisit', () => {
    it('Should throw if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(
        service.noOrderVisit('u1', { visitId: 'v1', reason: 'X' }),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('Should close visit with reason, GPS and emit socket', async () => {
      ...
      const res = await service.noOrderVisit('u1', {
        visitId: 'v1',
        reason: 'CLOSED_SHOP',
        note: 'shop was locked',
        latitude: 12.9,
        longitude: 77.5,
      });
      expect(res.status).toBe('CLOSED');
  ...
  describe('getVisits', () => {
    ...
    it('DISTRIBUTOR_ADMIN: should throw if distributor not found', async () => {
      ...
      await expect(
        service.getVisits('u1', 'DISTRIBUTOR_ADMIN'),
      ).rejects.toThrow(ForbiddenException);
    });

    it('MANUFACTURER_ADMIN: should return visits for linked distributors only', async () => {
      mockManufacturerRepo.findOne.mockResolvedValue({
        id: 'm1',
        user_id: 'u1',
      });
      mockManufacturerDistributorRepo.find.mockResolvedValue([
    ...
    it('MANUFACTURER_ADMIN: should throw if manufacturer not found', async () => {
      ...
      await expect(
        service.getVisits('u1', 'MANUFACTURER_ADMIN'),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('SALESMAN: should throw if salesman not found', async () => {
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(service.getVisits('u1', 'SALESMAN')).rejects.toThrow(
        ForbiddenException,
      );
    });

    it('Unknown role: should throw ForbiddenException', async () => {
      await expect(service.getVisits('u1', 'UNKNOWN_ROLE')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('getVisitById', () => {
    it('Should throw NotFoundException if visit does not exist', async () => {
      mockVisitRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getVisitById('u1', 'SUPER_ADMIN', 'v_missing'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('DISTRIBUTOR_ADMIN: should reject visit from different distributor (IDOR attempt)', async () => {
      ...
      await expect(
        service.getVisitById('u1', 'DISTRIBUTOR_ADMIN', 'v1'),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('MANUFACTURER_ADMIN: should allow access for linked ecosystem', async () => {
      ...
      mockManufacturerDistributorRepo.findOne.mockResolvedValue({
        manufacturer_id: 'm1',
        distributor_id: 'd1',
      });
    ...
    it('MANUFACTURER_ADMIN: should reject unlinked distributor visit (IDOR attempt)', async () => {
      ...
      await expect(
        service.getVisitById('u1', 'MANUFACTURER_ADMIN', 'v1'),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('SALESMAN: should reject access to another salesman visit (IDOR attempt)', async () => {
      ...
      await expect(
        service.getVisitById('u_attacker', 'SALESMAN', 'v1'),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('SALESMAN: should throw if salesman entity not found', async () => {
      ...
      mockSalesmanRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getVisitById('u1', 'SALESMAN', 'v1'),
      ).rejects.toThrow(ForbiddenException);
    });
  });
});
