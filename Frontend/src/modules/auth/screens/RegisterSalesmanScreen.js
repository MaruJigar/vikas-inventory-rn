...
import { RegisterSalesmanSchema } from '../validators/authSchemas';
import { useRegisterSalesmanMutation } from '../hooks/useAuthMutations';
import { COLORS, TYPOGRAPHY, SPACING, BORDER_RADIUS } from '../../../styles/colors';

export const RegisterSalesmanScreen = ({ navigation }) => {
  const [showPassword, setShowPassword] = useState(false);
  const registerMutation = useRegisterSalesmanMutation();

  const { control, handleSubmit, formState: { errors } } = useForm({
    resolver: zodResolver(RegisterSalesmanSchema),
    defaultValues: { full_name: '', email: '', phone: '', password: '', distributor_id: '' }
  });
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <Text style={styles.title}>Salesman Registration</Text>
          
          {['full_name', 'email', 'phone', 'distributor_id'].map((field) => (
