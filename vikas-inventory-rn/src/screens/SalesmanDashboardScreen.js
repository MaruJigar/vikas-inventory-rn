...
import {
  ...
  SafeAreaView, Platform, StatusBar, Alert, Linking
} from 'react-native';
...
export const SalesmanDashboardScreen = ({ navigation }) => {
  ...
  const handleCheckInOut = async () => {
    ...
    if (!loc) {
      Alert.alert('Error', 'Location is required for attendance');
      return;
    }
    
    try {
      if (isCheckedIn) {
        await ApiService.checkOut(loc.lat, loc.lng);
      } else {
        await ApiService.checkIn(loc.lat, loc.lng);
      }
      setIsCheckedIn(!isCheckedIn);
      Alert.alert('Success', isCheckedIn ? 'Checked out successfully' : 'Checked in successfully');
    } catch (error) {
  ...
  const handleOpenPDF = async () => {
    ...
    if (supported) {
      ...
      Alert.alert('Error', 'Unable to open PDF catalog on this device.');
    }
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.actionsGrid}>
            ...
            <QuickActionCard
              icon={isCheckedIn ? "✅" : "📍"}
              title={isCheckedIn ? t('dashboard.checkOut') : t('dashboard.checkIn')}
              subtitle="Attendance"
