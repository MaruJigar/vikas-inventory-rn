    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Submit Order',
          onPress: async () => {
            // Capture location here at order placement!
            const loc = await LocationService.getCurrentLocation();
            if (!loc) {
              Alert.alert('Error', 'Location capture is required to place an order.');
              return;
            }

            // Append location to the order submission
            const orderDataWithLocation = {
              location_lat: loc.lat,
              location_lng: loc.lng
            };
            
            // In the real api.js integration, submitOrder will accept this data.
            // For now, we simulate it with our AppContext mock:
            const order = submitOrder(orderDataWithLocation);
