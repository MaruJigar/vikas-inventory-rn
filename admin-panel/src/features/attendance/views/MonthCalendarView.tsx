...
export function MonthCalendarView({ data, isLoading, onCellClick, onSalesmanClick }: Props) {
  if (isLoading) {
    return (
              ...
              {daysHeader.map((h, i) => (
                <th key={i} scope="col" className="px-2 py-3 text-center border-r min-w-[64px]">
                  <div className="flex flex-col items-center">
                    <span className="text-muted-foreground font-normal">{h.dayName}</span>
                    <span className="font-bold text-sm">{h.dayNum}</span>
                  </div>
                </th>
              ))}
            </tr>
