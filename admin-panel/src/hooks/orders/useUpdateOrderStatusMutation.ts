...
import { UpdateOrderStatusDto } from '@/types/api/order.types';

export function useUpdateOrderStatusMutation(orderId: string | null) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: UpdateOrderStatusDto) => {
      if (!orderId) throw new Error('Order ID is required');
      return orderService.updateOrderStatus(orderId, data);
    },
    onSuccess: () => {
      handleSuccessToast('Order status updated successfully');
